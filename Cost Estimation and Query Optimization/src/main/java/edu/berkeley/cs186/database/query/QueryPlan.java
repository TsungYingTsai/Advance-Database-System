...
public class QueryPlan {
  ...
  public List<String> getSelectColumnNames() {return  this.selectColumnNames;}
  public List<String> getjoinTableNames() {return  this.joinTableNames;}
  public List<String> getjoinLeftColumnNames() {return  this.joinLeftColumnNames;}
  public List<String> getjoinRightColumnNames() {return  this.joinRightColumnNames;}
  ...
  public Iterator<Record> executeOptimal() throws DatabaseException, QueryPlanException {
    ...
    Map<Set,QueryOperator> Passmap = new HashMap<>();

    allTable.add(startTableName);
    for (String table:joinTableNames) {
      allTable.add(table);
    }

    for (String eachTable: allTable) {
      ...
      QueryOperator SCOp = minCostSingleAccess(eachTable);
      Set<String> tempSet = new HashSet<>();
      tempSet.add(eachTable);
      //if (SCOp.getIOCost() < minOp.getIOCost()) {
      //if (eachTable == startTableName) {
        map.put(tempSet, SCOp);
      //}
        Passmap.put(tempSet,SCOp);
    }
      ...
      // been joined.
    //TODO: I thinght! kkeep recursion until all tables are accounted
    for (int i = 0; i < allTable.size()-1; i += 1) {
      Map<Set,QueryOperator> tempMap = minCostJoins(map,Passmap);
      map = tempMap;
    }
    ...
    assert (map.size() == 1);
    assert (map.containsKey(allTable));
    QueryOperator ans = map.get(allTable);
    finalOperator = ans;
    ...
    this.addProjects();
      ...
      return finalOperator.execute(); //TODO: HW4 Replace this!!! Allows you to test intermediate functionality
    }
  ...
  public QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    QueryOperator newOp;
    List<Integer> IndexList = this.getEligibleIndexColumns(table);
    int ii = -1;

    for (int Index: IndexList) {
      String Col = selectColumnNames.get(Index);
      PredicateOperator operator = this.selectOperators.get(Index);
      DataBox value = this.selectDataBoxes.get(Index);

      newOp = new IndexScanOperator(this.transaction, table, Col, operator,
              value);

      int curCost = newOp.getIOCost();
      if (curCost < minCost) {
        ii = Index;
        minOp = newOp;
        minCost = curCost;
    ...
    minOp = addEligibleSelections(minOp, ii);
    return minOp;
  }
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
    ...
    for (Set prevSet: prevMap.keySet()) {
      //FOR EACH set of tables in prevMap:
      for (Set pass1Set : pass1Map.keySet()) {
        //FOR EACH join condition listed in the query
        /*
        if (Collections.disjoint(prevSet,pass1Set)) {
          for (String leftName : joinLeftColumnNames) {
            String leftColName = leftName.split("\\.")[1];
            String leftTableName = leftName.split("\\.")[0];
            for (String rightName : joinRightColumnNames) {
              String rightColName = rightName.split("\\.")[1];
              String rightTableName = rightName.split("\\.")[0];
...
*/
        for (int i = 0; i < this.joinTableNames.size(); i++){
              ...
              String leftColName = left[1];
              String rightTableName = right[0];
              String rightColName = right[1];

              if (prevSet.contains(leftTableName) == true && prevSet.contains(rightTableName) == false) {
                if (pass1Set.contains(rightTableName)) {
                  QueryOperator rightOp = pass1Map.get(pass1Set);
                  QueryOperator newOp = minCostJoinType(prevMap.get(prevSet), rightOp, leftColName, rightColName);
                  ...
                  if (map.containsKey(Un)) {
                    int curCost = map.get(Un).getIOCost();
                    if (newCost < curCost) {
                      map.put(Un, newOp);
                    }
                  } else {
              ...
              } else if (prevSet.contains(leftTableName) == false && prevSet.contains(rightTableName) == true) {
                //System.out.println("gjsgjskfjslkfjsdl");
                if (pass1Set.contains(leftTableName)) {
                  QueryOperator leftOp = pass1Map.get(pass1Set);
                  QueryOperator newOp = minCostJoinType(leftOp, prevMap.get(prevSet), leftColName, rightColName);
 ...
 *                 System.out.println("----------------------------------");
 ...
 System.out.println("prevSet : "+prevSet.toString());
 System.out.println("pass1Set : "+pass1Set.toString());
 System.out.println("leftTableName : "+leftTableName);
 System.out.println("rightTableName : "+rightTableName);
 System.out.println(pass1Set);
 System.out.println(pass1Set.contains(rightColName));
 System.out.println("----------------------------------");
 *
 */
}
