...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute){
    ...
    Iterator<Record> reIte = table.iterator();
    try {
      Record firstRecord = reIte.next();
      min = quantization(firstRecord,attribute);
      max = quantization(firstRecord,attribute);
    } catch (Exception e){
      throw new NotImplementedException();
    }
    while (reIte.hasNext()) {
      float compare = quantization(reIte.next(),attribute);
      min = compare < min? compare: min;
    ...
    Bucket<Float>[] curbuckets = new Bucket[numBuckets];

    for (int i = 0; i < curbuckets.length; i++) {
      curbuckets[i] = new Bucket(minValue+width*i,minValue+width*(i+1));
    }
    buckets = curbuckets;


    //4. populate the data using the increment(value) method
    Iterator<Record> reIteStore = table.iterator();
    while (reIteStore.hasNext()) {
      float increFloat = quantization(reIteStore.next(),attribute);
      int index = bucketIndex(increFloat);
      buckets[index].increment(increFloat);
    }
  }
  ...
  public Bucket<Float>[] getBucket() {return this.buckets;}
  ...
  private float [] allEquality(float qvalue){
    ...
    for (int i = 0; i < buckets.length; i +=1) {
      if (i != index) {
        ...
        result[i] = ((float)1/(float)buckets[i].getDistinctCount());
      }
    }
 ...
 /**
  ...
  private float [] allNotEquality(float qvalue){
    ...
    float [] equalResult = allEquality(qvalue);
    assert (result.length == equalResult.length);
    for (int i=0; i < result.length; i += 1) {
      result[i] = (float) 1 - equalResult[i];
    }
    return result;
  }
  ...
  private float [] allGreaterThan(float qvalue){
    //for (Bucket f: buckets) {System.out.println(f);}
    ...
    for (int i = 0; i < buckets.length; i +=1) {
      if (i < index) {
        ...
        assert (i == index);
        float end = buckets[i].getEnd();
        float start = buckets[i].getStart();
        float proportion = (end - qvalue)/width;
        result[i] = proportion;
      }
    }
  ...
  private float [] allLessThan(float qvalue){
    ...
    for (int i = 0; i < buckets.length; i +=1) {
      if (i > index) {
        ...
        float end = buckets[i].getEnd();
        float start = buckets[i].getStart();
        float proportion = (qvalue - start)/width;
        result[i] = proportion;
      }
    }
